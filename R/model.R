#' MB Model
#'
#' Creates object inherting from class mb_model.
#'
#' @param code Passed on to [pmb_code()]
...
pmb_model <- function(
  ...
  random_effects <- random_effects[sort(names(random_effects))]
  ...
  class(obj) <- sub("code", "model", class(code))
  obj
}

#' pmb_get_model
#'
#' Get the model object from an analysis.
#'
#' @param x An object of class `"mb_analysis"`
#'
#' @export
pmb_get_model <- function(x) {
  stopifnot(inherits(x, "mb_analysis"))
  x$model
}
